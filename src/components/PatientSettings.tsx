...
export function PatientSettings() {
  ...
  const [editForm, setEditForm] = useState({ name: '', email: '', nic: '', address: '', tel: '', password: '', cpassword: '' })
  const [msg, setMsg] = useState('')

  useEffect(() => {
    fetch('/api/patients?email=' + user?.email)
      .then(r => r.json())
      .then(data => {
        if (data.patients?.[0]) {
  ...
  const handleEdit = async () => {
    ...
    setMsg('')
    const res = await fetch(`/api/patients/${profile?.pid}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        pname: editForm.name,
        pemail: editForm.email,
        pnic: editForm.nic,
        paddress: editForm.address,
        ptel: editForm.tel,
        ppassword: editForm.password || undefined,
        oldEmail: profile?.pemail,
      }),
    })
  ...
  const handleDelete = async () => {
    if (!profile) return
    const res = await fetch(`/api/patients/${profile.pid}`, { method: 'DELETE' })
    if (res.ok) {
      await logout()
    }
  }
    ...
    <div className="space-y-6">
      ...
      {mode === 'view' && (
        <div className="rounded-xl border bg-white p-6 shadow-sm">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <InfoRow label="Patient ID" value={`P-${String(profile.pid).padStart(4, '0')}`} />
            <InfoRow label="Name" value={profile.pname} />
            <InfoRow label="Email" value={profile.pemail} />
            <InfoRow label="NIC" value={profile.pnic || 'N/A'} />
            <InfoRow label="Telephone" value={profile.ptel || 'N/A'} />
            <InfoRow label="Address" value={profile.paddress || 'N/A'} />
            <InfoRow label="Date of Birth" value={profile.pdob ? new Date(profile.pdob).toLocaleDateString() : 'N/A'} />
          </div>
        ...
        <div className="rounded-xl border bg-white p-6 shadow-sm">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              <InputField label="Address" value={editForm.address} onChange={v => setEditForm({ ...editForm, address: v })} />
            </div>
      ...
      {mode === 'delete' && (
        <div className="rounded-xl border border-red-200 bg-red-50 p-6">
          ...
          <p className="mb-4 text-sm text-red-600">Are you sure you want to permanently delete your account ({profile.pname})? This action cannot be undone.</p>
